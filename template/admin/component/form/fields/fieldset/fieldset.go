...
type Component struct {
	...
	Type string      `json:"type,omitempty"` // 支持 两种方式，type="group" 会用input.group 包裹;如果不配置 默认使用 space
...
// 支持 两种方式，type="group" 会用input.group 包裹;如果不配置 默认使用 space
func (p *Component) SetType(fieldsetFType string) *Component {
	p.Type = fieldsetFType
