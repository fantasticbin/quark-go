...
type Component struct {
	...
	IgnoreFormListField bool     `json:"ignoreFormListField,omitempty"` //
	Names               []string `json:"names,omitempty"`               // 组件内容
}
...
func (p *Component) SetWhen(value ...any) *Component {
	...
	if len(value) == 4 {
		name = value[0].(string)
		operator = value[1].(string)
		option = value[2]
		callback := value[3].(func() interface{})
	...
	case ">=":
		i.Condition = "<%=String(" + name + ") => '" + getOption + "' %>"
	case "has":
		i.Condition = "<%=(String(" + name + ").indexOf('" + getOption + "') !=-1) %>"
	case "in":
		jsonStr, _ := json.Marshal(option)
		i.Condition = "<%=(" + string(jsonStr) + ".indexOf(" + name + ") !=-1) %>"
	default:
		i.Condition = "<%=String(" + name + ") === '" + getOption + "' %>"
	}

	i.ConditionName = name
...
// 忽略FormList上的字段
func (p *Component) SetIgnoreFormListField(ignoreFormListField bool) *Component {
	p.IgnoreFormListField = ignoreFormListField
