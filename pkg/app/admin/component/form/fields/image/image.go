...
// 上传文件大小限制
func (p *Component) SetLimitSize(limitSize int) *Component {
	p.LimitSize = limitSize
	return p
}

// 上传文件类型限制
func (p *Component) SetLimitType(limitType []string) *Component {
	p.LimitType = limitType
	return p
}

// 上传文件数量限制
func (p *Component) SetLimitNum(limitNum int) *Component {
	p.LimitNum = limitNum
	return p
}

// 上传图片限制尺寸
func (p *Component) SetLimitWH(width int, height int) *Component {
	p.LimitWH = map[string]int{
		"width":  width,
		"height": height,
	}
...
// 上传按钮的标题
func (p *Component) SetButton(text string) *Component {
	p.Button = text
	return p
}
