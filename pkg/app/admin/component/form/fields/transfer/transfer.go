...
type Component struct {
	...
	Footer          interface{}            `json:"footer,omitempty"`          // 底部渲染函数
	ListStyle       map[string]interface{} `json:"listStyle,omitempty"`       // 两个穿梭框的自定义样式
	Locale          interface{}            `json:"locale,omitempty"`          // 各种语言,{ itemUnit: 项, itemsUnit: 项, searchPlaceholder: 请输入搜索内容 }
	OneWay          bool                   `json:"oneWay,omitempty"`          // 展示为单向样式
	Operations      []string               `json:"operations,omitempty"`      // 操作文案集合，顺序从上至下
	OperationStyle  interface{}            `json:"operationStyle,omitempty"`  // 操作栏的自定义样式
	Pagination      interface{}            `json:"pagination,omitempty"`      // 使用分页样式，自定义渲染列表下无效
	SelectAllLabels interface{}            `json:"selectAllLabels,omitempty"` // 自定义顶部多选框标题的集合
	SelectedKeys    []string               `json:"selectedKeys,omitempty"`    // 设置哪些项应该被选中
	ShowSearch      bool                   `json:"showSearch,omitempty"`      // 是否显示搜索框
	ShowSelectAll   bool                   `json:"showSelectAll,omitempty"`   // 是否展示全选勾选框
	Status          string                 `json:"status,omitempty"`          // 设置校验状态,'error' | 'warning'
	TargetKeys      []string               `json:"targetKeys,omitempty"`      // 显示在右侧框数据的 key 集合
	Titles          []string               `json:"titles,omitempty"`          // 标题集合，顺序从左至右
...
// 会在 label 旁增加一个 icon，悬浮后展示配置的信息，Column与Form共用此属性
...
// 自定义下拉菜单图标
func (p *Component) SetSelectionsIcon(selectionsIcon interface{}) *Component {
	p.SelectionsIcon = selectionsIcon

	return p
}

// 根据搜索内容进行筛选，接收 inputValue option 两个参数，当 option 符合筛选条件时，应返回 true，反之则返回 false
func (p *Component) SetFilterOption(filterOption interface{}) *Component {
	p.FilterOption = filterOption
...
// 两个穿梭框的自定义样式
func (p *Component) SetListStyle(listStyle map[string]interface{}) *Component {
...
func (p *Component) SetLocale(locale map[string]string) *Component {
	p.Locale = locale
...
// 展示为单向样式
func (p *Component) SetOneWay(oneWay bool) *Component {
	p.OneWay = oneWay
...
// 操作文案集合，顺序从上至下
func (p *Component) SetOperations(operations []string) *Component {
...
// 操作栏的自定义样式
func (p *Component) SetOperationStyle(operationStyle map[string]interface{}) *Component {
	p.OperationStyle = operationStyle

	return p
}

// 使用分页样式，自定义渲染列表下无效
func (p *Component) SetPagination(pagination interface{}) *Component {
...
// 自定义顶部多选框标题的集合
func (p *Component) SetSelectAllLabels(selectAllLabels interface{}) *Component {
	p.SelectAllLabels = selectAllLabels

	return p
}

// 设置哪些项应该被选中
func (p *Component) SetSelectedKeys(selectedKeys []string) *Component {
...
// 是否展示全选勾选框
func (p *Component) SetShowSelectAll(showSelectAll bool) *Component {
	p.ShowSelectAll = showSelectAll
...
// 显示在右侧框数据的 key 集合
func (p *Component) SetTargetKeys(targetKeys []string) *Component {
	p.TargetKeys = targetKeys

	return p
}

// 标题集合，顺序从左至右
func (p *Component) SetTitles(titles []string) *Component {
	p.Titles = titles
