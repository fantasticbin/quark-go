...
type Captcha struct {
	Size string `json:"size,omitempty"` // 验证码按钮大小
	Text string `json:"text,omitempty"` // 验证码按钮文案
	Url  string `json:"url,omitempty"`  // 获取验证码请求地址
}
...
type Component struct {
	...
	Dependency   string   `json:"dependency,omitempty"`   // 依赖的字段名称，默认为phone
	CaptchaProps *Captcha `json:"captchaProps,omitempty"` // 验证码属性
}
...
// 设置依赖的字段名称，默认为phone
func (p *Component) SetDependency(dependency string) *Component {
...
// 设置验证码属性
func (p *Component) SetCaptchaProps(captchaProps *Captcha) *Component {
	p.CaptchaProps = captchaProps
